...
if [ ! -d "./preprocessed_checkpoints" ]; then
    echo "❌ エラー: 事前処理チェックポイントディレクトリが見つかりません"
    echo "   create_non_distributed_checkpoint.py を先に実行してください"
    exit 1
fi

# 最新のチェックポイントを検索
LATEST_CHECKPOINT=$(find ./preprocessed_checkpoints -name "lisa_gemma_non_distributed_*" -type d | sort | tail -1)
