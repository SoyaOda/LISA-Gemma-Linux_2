...
def setup_wandb_auth(ip):
    ...
    if not wandb_token:
        # wandb_api_key.txtファイルから確認（新しく追加）
        try:
    ...
    # Lambda Cloud上でWandB認証を設定
    auth_command = f"echo '{wandb_token}' | wandb login --relogin"
    success, stdout, stderr = run_ssh_command(ip, auth_command)
    ...
    if success:
        ...
        # 環境変数方式でも試す
        env_command = f"export WANDB_API_KEY='{wandb_token}' && echo 'export WANDB_API_KEY={wandb_token}' >> ~/.bashrc"
        success2, _, _ = run_ssh_command(ip, env_command)
        
        if success2:
            print("   ✅ WandB認証設定完了（環境変数方式）")
            return True
        else:
...
def create_setup_aliases(ip):
    ...
    aliases = f'''
# LISA-Gemma Lambda Cloud エイリアス
alias lc-sync='rsync -avz --exclude=".git" --exclude="__pycache__" --exclude="*.pyc" --exclude=".gitignore" -e "ssh -i ~/.ssh/lambda_cloud_key"'
alias lc-run='ssh -i ~/.ssh/lambda_cloud_key ubuntu@{ip} "cd /lambda/nfs/lisa-gemma-project-fs/code/LISA-Gemma-Linux && source ../../venvs/lisa_gemma_venv/bin/activate && export TF_CPP_MIN_LOG_LEVEL=3 && export TF_ENABLE_ONEDNN_OPTS=0 &&"'
